...
The verdict: **the shape holds, and the exercise paid for itself.** Five
...
- **Converter-carrying generics.** `'a StateField.t`, `'a RangeSet.t`,
  ...
  The cost is real but small: these types cannot be `Jv.CONV`, so they
  expose `conv` and `to_jv` individually, and a reader of the interface
  cannot see which types are plain handles and which are records. That is
...
3. **Three separate packages could not name a type CodeMirror itself
   defines.** `view` needed `EditorView.announce`, an effect type
   CodeMirror dispatches internally; `language` needed `foldState`;
   `commands` needed `historyField`. Defining a fresh one produces a
   different JavaScript object, so it never matches. The view agent
   reached for `Obj.magic` over another package's private record, which
   worked and would have rotted. `StateEffectType`, `AnnotationType` and
   `StateField` now all expose `of_jv`, and the `Obj.magic` is gone.
4. **A state field's JSON hooks without the state.** Cheap to add,
   impossible to work around, and a field that resolves against a
   sibling needs it.
5. **Two of the conventions were simply wrong as written**, on when a
