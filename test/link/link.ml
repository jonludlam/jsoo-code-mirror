...
let () =
  ...
  let same_state_class =
    Jv.instanceof (Cm_state.EditorState.to_jv s) ~cons:(Jv.get (Jv.get Jv.global "__CM__state") "EditorState")
  in
  let globals = List.filter (fun g -> not (Jv.is_undefined (Jv.get Jv.global g)))
      [ "__CM__state"; "__CM__view"; "__CM__language"; "__CM__commands"; "__CM__autocomplete"; "__CM__lint"; "__CM__search"; "__CM__legacy_modes"; "__CM__theme_one_dark"; "__CM__codemirror" ] in
  let basic = not (Jv.is_undefined (Code_mirror.basic_setup ())) in
  Jv.set Jv.global "linkResult"
    (Jv.obj [| ("doc", Jv.of_string (Cm_state.Text.to_string (Cm_state.EditorState.doc s)));
