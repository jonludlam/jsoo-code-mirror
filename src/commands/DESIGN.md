...
- **`Conv.callback`, added in `Cm_state` for exactly this shape, had no
  ...
  still stands.
- **`isolate_history`/`AnnotationType.of_jv` and `inverted_effects`/
  `Facet.of_jv` are the payoff for two of `Cm_state`'s own friction-driven
  fixes.** `AnnotationType.of_jv` didn't exist until `Cm_view` needed the
  `StateEffectType` version for `EditorView.announce`; picking up the
  `AnnotationType` half here for `isolateHistory` (a CodeMirror-defined
  annotation type, not one this package's own `AnnotationType.define`
  could produce with the right JavaScript identity) confirms the general
  fix was the right one, not a one-off.
- **`history_field : Jv.t` is an honest dead end, not a design gap this
  package could close.** `Cm_state.StateField.t` has no public constructor
  from a raw JavaScript `StateField` (only `StateField.define` makes one),
  and `EditorState.to_json`/`from_json` don't take the per-field
  dictionary that is the *only* place JavaScript itself threads a
  `StateField` like this one through. So `history_field` is bound as a
  bare handle with a doc comment explaining why nothing more typed is
  possible, the same call `@codemirror/language`'s `folded_ranges` doc
  comment already made for `foldState`. Two packages independently hitting
  the same wall is worth noting in `Cm_state`'s own "Not bound" list
  rather than re-discovering it a third time.
