...
(* A typed Jv value, after patricoferris/jsoo-code-mirror#17: the one
   representation behind every generic CodeMirror type bound here
   ([StateField<T>], [RangeSet<T>], ...), a JavaScript handle and the
   converter for its payload. Each module keeps its [t] abstract and takes
   its accessors from [include (Tjv.Id : Tjv.CONV ...)]. *)
module Tjv = struct
  type 'a conv = 'a Conv.t = { to_jv : 'a -> Jv.t; of_jv : Jv.t -> 'a }

  let conv to_jv of_jv = { to_jv; of_jv }

  type 'a t = { jv : Jv.t; payload : 'a conv }

  let map_jv t f = { t with jv = f t.jv }
  ...
  module Id = struct
    let to_jv t = t.jv
    let of_jv payload jv = { jv; payload }
    let conv t = t.payload
    let any t = { jv = t.jv; payload = Conv.jv }
    let conv_of c = { to_jv; of_jv = of_jv c }
  end
end
...
module StateEffectType = struct
  ...
  let define (type a) ?map (conv : a Conv.t) : a t =
    ...
    Tjv.Id.of_jv conv jv

  let of_ (t : 'a t) (v : 'a) : state_effect =
    Jv.call (Tjv.Id.to_jv t) "of" [| (Tjv.Id.conv t).to_jv v |]

  let reconfigure : Extension.t t =
    Tjv.Id.of_jv Extension.conv
      (Jv.get (Lazy.force state_effect_cls) "reconfigure")

  let append_config : Extension.t t =
    Tjv.Id.of_jv Extension.conv
      (Jv.get (Lazy.force state_effect_cls) "appendConfig")
end
...
module StateEffect = struct
  ...
  let is (e : t) (ty : 'a StateEffectType.t) : bool =
    Jv.call e "is" [| Tjv.Id.to_jv ty |] |> Jv.to_bool

  let value (e : t) (ty : 'a StateEffectType.t) : 'a option =
    if is e ty then Some ((Tjv.Id.conv ty).of_jv (Jv.get e "value")) else None
...
module Facet = struct
  ...
  let from (type a) ?get (f : ('i, 'o) t) (field : a state_field) : Extension.t
      =
    match get with
    | None -> Jv.call f.facet_jv "from" [| Tjv.Id.to_jv field |]
    | Some g ->
        let wrapped (v : Jv.t) =
          f.in_conv.to_jv (g ((Tjv.Id.conv field).of_jv v))
        in
        Jv.call f.facet_jv "from"
          [| Tjv.Id.to_jv field; Jv.callback ~arity:1 wrapped |]

  let dep_to_jv = function
    | Doc -> Jv.of_string "doc"
    | Selection -> Jv.of_string "selection"
    | Facet_dep fd -> fd.facet_jv
    | Field_dep sf -> Tjv.Id.to_jv sf
...
module StateField = struct
  ...
  let define (type a) ?compare ?provide ?to_json ?from_json (conv : a Conv.t)
    ...
    Option.iter
      (fun f ->
        let wrapped (raw : Jv.t) =
          Extension.to_jv (f (Tjv.Id.of_jv conv raw))
        in
    ...
    Tjv.Id.of_jv conv field_jv

  let extension (f : 'a t) : Extension.t = Jv.get (Tjv.Id.to_jv f) "extension"

  let init (f : 'a t) (init : editor_state -> 'a) : Extension.t =
    let wrapped (st : Jv.t) = (Tjv.Id.conv f).to_jv (init st) in
    Jv.call (Tjv.Id.to_jv f) "init" [| Jv.callback ~arity:1 wrapped |]
end
...
module Range = struct
  ...
  let from (r : 'a t) = Jv.Int.get (Tjv.Id.to_jv r) "from"
  let to_ (r : 'a t) = Jv.Int.get (Tjv.Id.to_jv r) "to"
  let value (r : 'a t) = (Tjv.Id.conv r).of_jv (Jv.get (Tjv.Id.to_jv r) "value")

  let make (type a) (c : a Conv.t) ~from ~to_ (v : a) : a t =
    Tjv.Id.of_jv c
      (Jv.call (c.to_jv v) "range" [| Jv.of_int from; Jv.of_int to_ |])
end
...
module RangeSet = struct
  ...
  let empty (type a) (c : a Conv.t) : a t =
    Tjv.Id.of_jv c (Jv.get (Lazy.force range_set_cls) "empty")

  let of_ (type a) ?sort (c : a Conv.t) (ranges : a Range.t list) : a t =
    let arr = Jv.of_list (fun (r : a Range.t) -> Tjv.Id.to_jv r) ranges in
    Tjv.Id.of_jv c
      (Jv.call (Lazy.force range_set_cls) "of" [| arr; opt_bool sort |])

  let size (t : 'a t) = Jv.Int.get (Tjv.Id.to_jv t) "size"
  ...
  let update (type a) ?add ?sort ?filter ?filter_from ?filter_to (t : a t) : a t
    ...
    | None -> ()
    | Some rs ->
        Jv.set o "add" (Jv.of_list (fun (r : a Range.t) -> Tjv.Id.to_jv r) rs));
    ...
    Tjv.map_jv t (fun jv -> Jv.call jv "update" [| o |])

  let map (t : 'a t) (cd : ChangeDesc.t) : 'a t =
    Tjv.map_jv t (fun jv -> Jv.call jv "map" [| cd |])
  ...
  let between ?from ?to_ (t : 'a t) f =
    ...
    let wrapped (fa : Jv.t) (ta : Jv.t) (v : Jv.t) =
      if f ~from:(Jv.to_int fa) ~to_:(Jv.to_int ta) ((Tjv.Id.conv t).of_jv v)
      then Jv.undefined
      else Jv.false'
    in
    Jv.call (Tjv.Id.to_jv t) "between"
      [| Jv.of_int from; Jv.of_int to_; Jv.callback ~arity:3 wrapped |]
    |> ignore

  let iter ?from (t : 'a t) (f : 'a Range.t -> unit) : unit =
    let cur = Jv.call (Tjv.Id.to_jv t) "iter" [| opt_int from |] in
    let rec loop () =
      ...
      if not (Jv.is_null v) then (
        let snapshot =
          Jv.obj
            ...
            |]
        in
        f (Tjv.Id.of_jv (Tjv.Id.conv t) snapshot);
  ...
  let eq ?from ?to_ (a : 'a t) (b : 'a t) : bool =
    Jv.call (Lazy.force range_set_cls) "eq"
      [|
        Jv.of_list Fun.id [ Tjv.Id.to_jv a ];
        Jv.of_list Fun.id [ Tjv.Id.to_jv b ];
        opt_int from;
        opt_int to_;
      |]
    |> Jv.to_bool
  ...
  let join (type a) (c : a Conv.t) (sets : a t list) : a t =
    let arr = Jv.of_list (fun (s : a t) -> Tjv.Id.to_jv s) sets in
    Tjv.Id.of_jv c (Jv.call (Lazy.force range_set_cls) "join" [| arr |])
end
...
module RangeSetBuilder = struct
  ...
  let make (type a) (c : a Conv.t) () : a t =
    Tjv.Id.of_jv c (Jv.new' (Lazy.force range_set_builder_cls) [||])

  let add (t : 'a t) ~from ~to_ (v : 'a) =
    Jv.call (Tjv.Id.to_jv t) "add"
      [| Jv.of_int from; Jv.of_int to_; (Tjv.Id.conv t).to_jv v |]
    |> ignore

  let finish (t : 'a t) : 'a RangeSet.t =
    Tjv.Id.of_jv (Tjv.Id.conv t) (Jv.call (Tjv.Id.to_jv t) "finish" [||])
end
...
module AnnotationType = struct
  ...
  let define (type a) (conv : a Conv.t) : a t =
    Tjv.Id.of_jv conv (Jv.call (Lazy.force annotation_cls) "define" [||])
...
module Annotation = struct
  ...
  let value (ann : t) (ty : 'a AnnotationType.t) : 'a option =
    if Jv.strict_equal (Jv.get ann "type") (Tjv.Id.to_jv ty) then
      Some ((Tjv.Id.conv ty).of_jv (Jv.get ann "value"))
    else None
end
...
module Transaction = struct
  ...
  let annotation (t : t) (ty : 'a AnnotationType.t) : 'a option =
    let r = Jv.call t "annotation" [| Tjv.Id.to_jv ty |] in
    if Jv.is_undefined r then None else Some ((Tjv.Id.conv ty).of_jv r)

  let is_user_event t ev =
    Jv.call t "isUserEvent" [| Jv.of_string ev |] |> Jv.to_bool

  let time : int AnnotationType.t =
    AnnotationType.of_jv Conv.int (Jv.get (Lazy.force transaction_cls) "time")

  let user_event : string AnnotationType.t =
    AnnotationType.of_jv Conv.string
      (Jv.get (Lazy.force transaction_cls) "userEvent")

  let add_to_history : bool AnnotationType.t =
    AnnotationType.of_jv Conv.bool
      (Jv.get (Lazy.force transaction_cls) "addToHistory")

  let remote : bool AnnotationType.t =
    AnnotationType.of_jv Conv.bool
      (Jv.get (Lazy.force transaction_cls) "remote")
end
...
module EditorState = struct
  ...
  let field (t : t) (f : 'a state_field) : 'a =
    let v = Jv.call t "field" [| Tjv.Id.to_jv f; Jv.false' |] in
    if Jv.is_undefined v then Conv.invalid "EditorState.field" t
    else (Tjv.Id.conv f).of_jv v

  let field_opt (t : t) (f : 'a state_field) : 'a option =
    let v = Jv.call t "field" [| Tjv.Id.to_jv f; Jv.false' |] in
    if Jv.is_undefined v then None else Some ((Tjv.Id.conv f).of_jv v)
