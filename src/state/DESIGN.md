...
(** Typed values, after patricoferris/jsoo-code-mirror#17: the one
    representation and signature every single-parameter container below
    shares. Each keeps its own abstract ['a t]. *)
module Tjv : sig
  ...
  module type CONV = sig
    type 'a t
    val to_jv : 'a t -> Jv.t
    val of_jv : 'a conv -> Jv.t -> 'a t   (** wraps a value another package made *)
    val conv : 'a t -> 'a conv            (** the payload's converter *)
    val any : 'a t -> Jv.t t
    val conv_of : 'a conv -> 'a t conv    (** the container's own converter *)
  end
end
