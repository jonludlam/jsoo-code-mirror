...
- **The `foldState` gap: a `StateField.t` this package cannot construct.**
  `@codemirror/language` exports a ready-made `StateField<DecorationSet>`
  (`foldState`) the same way `@codemirror/view` exports `announce` (a ready
  `StateEffectType`) — but unlike `StateEffectType`/`AnnotationType`, which
  DECISIONS.md already fixed with a public `of_jv`, `Cm_state.StateField.t`
  has no such escape hatch; `StateField.define` is its only constructor.
  This package works around it by not exposing `fold_state` at all and
  relying on `folded_ranges` (the free function reading the same
  information) instead, but the underlying gap is real and will recur for
  any future package that needs to name a state field CodeMirror itself
  defines rather than one bound by this library. Worth the same fix
  `StateEffectType`/`AnnotationType` got.
