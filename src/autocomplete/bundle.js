(() => {
  ...
  var FieldRange = class _FieldRange {
    ...
    map(changes) {
      let from = changes.mapPos(this.from, -1, import_state.MapMode.TrackDel);
      let to = changes.mapPos(this.to, 1, import_state.MapMode.TrackDel);
      ...
      for (let line of this.lines) {
        if (text.length) {
          ...
          for (let i = 0; i < tabs; i++)
            indent += state.facet(import_language.indentUnit);
  ...
  var setActive = /* @__PURE__ */ import_state.StateEffect.define({
    map(value, changes) {
      return value && value.map(changes);
    }
  });
  var moveToField = /* @__PURE__ */ import_state.StateEffect.define();
  var snippetState = /* @__PURE__ */ import_state.StateField.define({
    create() {
      return null;
    },
    update(value, tr) {
    ...
    provide: (f) => import_view.EditorView.decorations.from(f, (val) => val ? val.deco : import_view.Decoration.none)
  });
  function fieldSelection(ranges, field) {
    return import_state.EditorSelection.create(ranges.filter((r) => r.field == field).map((r) => import_state.EditorSelection.range(r.from, r.to)));
  }
  function snippet(template) {
    ...
    return (editor, completion, from, to) => {
      ...
      let spec = {
        changes: { from, to: to == main.from ? main.to : to, insert: import_state.Text.of(text) },
        scrollIntoView: true,
        annotations: completion ? [pickedCompletion.of(completion), import_state.Transaction.userEvent.of("input.complete")] : void 0
      };
      if (ranges.length)
      ...
      if (ranges.some((r) => r.field > 0)) {
        ...
        if (editor.state.field(snippetState, false) === void 0)
          effects.push(import_state.StateEffect.appendConfig.of([snippetState, addSnippetKeymap, snippetPointerHandler, baseTheme]));
      }
  ...
  var snippetKeymap = /* @__PURE__ */ import_state.Facet.define({
    combine(maps) {
  ...
  var closeBracketEffect = /* @__PURE__ */ import_state.StateEffect.define({
    map(value, mapping) {
      let mapped = mapping.mapPos(value, -1, import_state.MapMode.TrackAfter);
  ...
  var bracketState = /* @__PURE__ */ import_state.StateField.define({
    create() {
  ...
  function closing(ch) {
    ...
    return (0, import_state.fromCodePoint)(ch < 128 ? ch : ch + 1);
  }
  function config(state, pos) {
    return state.languageDataAt("closeBrackets", pos)[0] || defaults;
  }
  var android = typeof navigator == "object" && /* @__PURE__ */ /Android\b/.test(navigator.userAgent);
  var inputHandler = /* @__PURE__ */ import_view.EditorView.inputHandler.of((view, from, to, insert) => {
    if ((android ? view.composing : view.compositionStarted) || view.state.readOnly)
      return false;
    let sel = view.state.selection.main;
    if (insert.length > 2 || insert.length == 2 && (0, import_state.codePointSize)((0, import_state.codePointAt)(insert, 0)) == 1 || from != sel.from || to != sel.to)
      return false;
    ...
    let dont = null, changes = state.changeByRange((range) => {
      if (range.empty) {
        ...
        for (let token of tokens) {
          if (token == before && nextChar(state.doc, range.head) == closing((0, import_state.codePointAt)(token, 0)))
            return {
              changes: { from: range.head - token.length, to: range.head + token.length },
              range: import_state.EditorSelection.cursor(range.head - token.length)
            };
        }
      }
  ...
  function insertBracket(state, bracket) {
    ...
    for (let tok of tokens) {
      let closed = closing((0, import_state.codePointAt)(tok, 0));
      if (bracket == tok)
  ...
  function nextChar(doc, pos) {
    ...
    return next.slice(0, (0, import_state.codePointSize)((0, import_state.codePointAt)(next, 0)));
  }
  function prevChar(doc, pos) {
    let prev = doc.sliceString(pos - 2, pos);
    return (0, import_state.codePointSize)((0, import_state.codePointAt)(prev, 0)) == prev.length ? prev : prev.slice(1);
  }
    ...
    let dont = null, changes = state.changeByRange((range) => {
      if (!range.empty)
        return {
          ...
          range: import_state.EditorSelection.range(range.anchor + open.length, range.head + open.length)
        };
      ...
      if (!next || /\s/.test(next) || closeBefore.indexOf(next) > -1)
        return {
          ...
          range: import_state.EditorSelection.cursor(range.head + open.length)
        };
      return { range: dont = range };
    });
    ...
    let dont = null, changes = state.changeByRange((range) => {
      if (range.empty && nextChar(state.doc, range.head) == close)
        return {
          ...
          range: import_state.EditorSelection.cursor(range.head + close.length)
        };
      return dont = { range };
    });
    return dont ? null : state.update(changes, {
    ...
    let dont = null, changes = state.changeByRange((range) => {
      if (!range.empty)
        return {
          ...
          range: import_state.EditorSelection.range(range.anchor + token.length, range.head + token.length)
        };
      ...
      if (next == token) {
        if (nodeStart(state, pos)) {
          return {
            ...
            range: import_state.EditorSelection.cursor(pos + token.length)
          };
          ...
          return {
            ...
            range: import_state.EditorSelection.cursor(pos + content.length)
          };
        }
      ...
      } else if (state.charCategorizer(pos)(next) != import_state.CharCategory.Word) {
  ...
  function nodeStart(state, pos) {
    let tree = (0, import_language.syntaxTree)(state).resolveInner(pos + 1);
  ...
  function canStartStringAt(state, pos, prefixes) {
    ...
    if (charCat(state.sliceDoc(pos - 1, pos)) != import_state.CharCategory.Word)
      return pos;
    for (let prefix of prefixes) {
      let start = pos - prefix.length;
      if (state.sliceDoc(start, pos) == prefix && charCat(state.sliceDoc(start - 1, start)) != import_state.CharCategory.Word)
        return start;
    }
    return -1;
  }
  ...
  // js/entries/autocomplete.js
  globalThis.__CM__autocomplete = dist_exports;
})();
