(() => {
  ...
  var WidgetView = class _WidgetView extends ContentView {
    ...
    get overrideDOMText() {
      ...
      return text ? text.slice(start, start + this.length) : import_state.Text.empty;
    }
    domAtPos(pos) {
  ...
  var BlockWidgetView = class _BlockWidgetView extends ContentView {
    ...
    get overrideDOMText() {
      return this.parent ? this.parent.view.state.doc.slice(this.posAtStart, this.posAtEnd) : import_state.Text.empty;
    }
    domBoundsAround() {
      return null;
    }
  ...
  var ContentBuilder = class _ContentBuilder {
    ...
    static build(text, from, to, decorations2, dynamicDecorationMap) {
      ...
      builder.openEnd = import_state.RangeSet.spans(decorations2, from, to, builder);
  ...
  var DocView = class extends ContentView {
    ...
    coordsForChar(pos) {
      ...
      if (!(child instanceof TextView))
        return null;
      let end = (0, import_state.findClusterBreak)(child.text, off);
      if (end == off)
        return null;
  ...
  function groupAt(state, pos, bias = 1) {
    ...
    return import_state.EditorSelection.range(from + line.from, to + line.from);
  }
  ...
  function moveToLineBoundary(view, start, forward, includeWrap) {
    ...
    return import_state.EditorSelection.cursor(forward ? line.to : line.from, forward ? -1 : 1);
  }
  ...
  function moveVertically(view, start, forward, distance) {
    ...
    for (let extra = 0; ; extra += 10) {
      ...
      if (curY < rect.top || curY > rect.bottom || (dir < 0 ? pos < startPos : pos > startPos)) {
        ...
        return import_state.EditorSelection.cursor(pos, assoc, void 0, goal);
      }
    }
  }
  ...
  var DOMReader = class {
    constructor(points, state) {
      this.points = points;
      this.text = "";
      this.lineSeparator = state.facet(import_state.EditorState.lineSeparator);
    }
    append(text) {
      this.text += text;
    }
  ...
  function selectionFromPoints(points, base2) {
    ...
    return anchor > -1 && head > -1 ? import_state.EditorSelection.single(anchor + base2, head + base2) : null;
  }
    ...
    if (linewise) {
      ...
      changes = state.changeByRange((range) => {
        ...
        return {
          ...
          range: import_state.EditorSelection.cursor(range.from + insert.length)
        };
      });
    } else if (byLine) {
      changes = state.changeByRange((range) => {
        ...
        return {
          ...
          range: import_state.EditorSelection.cursor(range.from + line.length)
        };
      });
    } else {
  ...
  function rangeForClick(view, pos, bias, type) {
    if (type == 1) {
      ...
      if (to < view.state.doc.length && to == line.to)
        to++;
      return import_state.EditorSelection.range(from, to);
    }
  }
    ...
    return {
      ...
      get(event2, extend, multiple) {
        ...
        if (start.pos != cur.pos && !extend) {
          ...
          range = from < range.from ? import_state.EditorSelection.range(from, to) : import_state.EditorSelection.range(to, from);
        }
        if (extend)
    ...
    if (event.target.draggable) {
      ...
      if (cView && cView.isWidget) {
        ...
        if (from >= range.to || to <= range.from)
          range = import_state.EditorSelection.range(from, to);
      }
    }
    let { inputState } = view;
  ...
  var EditContextManager = class {
    constructor(view) {
      ...
      this.handlers.textupdate = (e) => {
        ...
        let change = { from, to, insert: import_state.Text.of(e.text.split("\n")) };
  ...
  var EditorView = class _EditorView {
    ...
    */
    constructor(config = {}) {
      ...
      this.viewState = new ViewState(config.state || import_state.EditorState.create(config));
      ...
      try {
        ...
        for (let tr of transactions) {
          ...
          if (tr.scrollIntoView) {
            ...
            scrollTarget = new ScrollTarget(main.empty ? main : import_state.EditorSelection.cursor(main.head, main.head > main.anchor ? -1 : 1));
          }
    ...
    */
    visualLineSide(line, end) {
      ...
      return import_state.EditorSelection.cursor(span.side(end, dir) + line.from, span.forward(!end, dir) ? 1 : -1);
    }
    /**
    ...
    */
    static scrollIntoView(pos, options = {}) {
      return scrollIntoView.of(new ScrollTarget(typeof pos == "number" ? import_state.EditorSelection.cursor(pos) : pos, options.y, options.x, options.yMargin, options.xMargin));
    }
    /**
    ...
    markers(view) {
      ...
      for (let r of state.selection.ranges) {
        ...
        if (r.empty || conf.drawRangeCursor) {
          ...
          let cursor = r.empty ? r : import_state.EditorSelection.cursor(r.head, r.head > r.anchor ? -1 : 1);
