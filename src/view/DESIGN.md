...
- **`Conv`-carrying generics worked well for `Decoration.t RangeSet.t`
  ...
  `Decoration.t RangeSet.t` slots straight into `Cm_state.RangeSet.conv
...
- **`StateEffectType` has no way, from outside `Cm_state`, to wrap a
  pre-existing JavaScript `StateEffectType`.** `EditorView.announce` is a
  concrete `StateEffectType<string>` CodeMirror already constructed
  internally; binding it needs a `'a StateEffectType.t` value that wraps
  *that* JavaScript object, not a new one from `StateEffectType.define`
  (a fresh `define` would produce a different JavaScript identity, so
  `StateEffect.is`/`value` against it would never match what CodeMirror
  actually dispatches). `Facet` exposes `of_jv` for exactly this
  situation; `StateEffectType` and `AnnotationType` do not. The
  workaround here (`src/view/cm_view.ml`, `EditorView.announce`) is an
  `Obj.magic` through a locally-declared mirror of `Cm_state`'s private
  `{ se_jv; se_conv }` record — safe under js_of_ocaml's uniform block
  representation, but it is exactly the kind of implementation-detail
  reliance the `Conv`/module-abstraction design is supposed to make
  unnecessary. A `StateEffectType.of_jv : 'a Conv.t -> Jv.t -> 'a t` (and
  `AnnotationType.of_jv`, for the same reason) would remove the need for
  it entirely.
