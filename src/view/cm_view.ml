...
module ViewUpdate = struct
  ...
  let view_changed t = Jv.Bool.get t "viewportChanged"
...
module Decoration = struct
  ...
  let none : t RangeSet.t =
    (RangeSet.conv conv).of_jv (Jv.get (Lazy.force decoration_cls) "none")
    ...
    Option.iter
      (fun f ->
        let wrapped (w : Jv.t) =
          ...
          (RangeSet.conv Decoration.conv).to_jv (f a_val)
        in
      ...
      Conv.to_jv =
        (fun f ->
          Jv.callback ~arity:1 (fun (v : Jv.t) ->
              (RangeSet.conv Decoration.conv).to_jv (f v)));
      of_jv =
        (fun jv (v : editor_view) ->
          (RangeSet.conv Decoration.conv).of_jv (Jv.apply jv [| v |]));
    }
  ...
  Option.iter
    (fun f ->
      let wrapped (view : Jv.t) =
        (RangeSet.conv GutterMarker.conv).to_jv (f view)
      in
...
let line_number_markers : (GutterMarker.t RangeSet.t, Jv.t) Facet.t =
  Facet.of_jv
    (RangeSet.conv GutterMarker.conv)
    Conv.jv
